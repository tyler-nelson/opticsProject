...
class Mirror:
    ...
    def predefined(cls, name, lossFunction=lambda i: 0, xOffSet=0, yOffSet=0):
        ...
        if(name == "concave up"):
            ...
            return cls([[[xOffSet, 3+xOffSet], [f]]],
                       lossFunction=lossFunction)
        elif(name == "concave down"):
            f = partial(circle, False, 5+yOffSet, 16, xOffSet)
            return cls([[[xOffSet, 4+xOffSet], [f]]],
                       lossFunction=lossFunction)
        elif(name == "horizontal"):
            f = partial(line, 4 + yOffSet)
            return cls([[[xOffSet, 5+xOffSet], [f]]],
                       lossFunction=lossFunction)
        elif(name == "vertical"):
            return cls([[[3+xOffSet], [1+yOffSet, 6+yOffSet]]],
                       lossFunction=lossFunction)
        elif(name == "parabolic down"):
            return cls([[[xOffSet, 10+xOffSet],
    ...
    def gaussianOptics(cls, R=0, vertical=0, diameter=0, xOffSet=0, yOffSet=0,
        ...
        if(diameter or R):
            if(R > 0):
                ...
                if(vertical):
                    return cls([[[xOffSet], [yOffSet-diameter/2,
                                                         yOffSet+diameter/2]]])
                return cls([[[xOffSet-diameter/2,
                                          xOffSet+diameter/2],
                                         [lambda x: 3+yOffSet]]])
    ...
    def custom(cls, surfaceList, lossFunction=lambda i: 0):
        return cls(surfaceList, lossFunction=lossFunction)
