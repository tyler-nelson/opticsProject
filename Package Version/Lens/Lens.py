...
class Lens:
    ...
    def predefined(cls, name, nFunction=lambda x: 1.5, xOffSet=0, yOffSet=0,
        ...
        ***NOTE***
        ...
        if(name == "biconvex"):
            ...
            return cls(surfaceList=[[[x0, x1], [f1, f2]]], n=nFunction)
        elif(name == "biconcave"):
            f1 = partial(circle, False, 1 + yOffSet, 3, 5-xOffSet)
        ...
        ***NOTE***
        ...
        def abrvFunc(r1, r2, f1, f2, diameter, x0=0, x1=0, xOffSet=0,
            ...
            if(x0 or x1) and abs(x1-x0) <= diameter:
                raise(Exception("this diameter exceeds the limitations \
                                imposed by the specified diameter"))
            xmin = rmax - diameter/2
            xmax = rmax + diameter/2
            mySurfaceList = [[[xmin+xOffSet],
                              sorted([f1(xmin+xOffSet), f2(xmax+xOffSet)])]]
            mySurfaceList.append([[xmin+xOffSet, xmax+xOffSet], [f1, f2]])
            mySurfaceList.append([[xmax+xOffSet],
                                  sorted([complex(f1(xmax+xOffSet)).real,
                                          complex(f2(xmax+xOffSet)).real])])
        ...
        if(((r1 or r2 or diameter) and thickness)
           ...
           or (r1 and r2)):
            ...
            if(thickness < 0 or diameter < 0):
                ...
                #  meniscus convex case 2 #fixed I think
                if(not thickness):
                    thickness = abs(r1-(r1**2 - diameter**2/4)**(1/2) - r2 +
                                    (r2**2-diameter**2/4)**(1/2))
                r1, r2 = -r1, -r2
                rmax = r1
                f1 = partial(circle, True, 1 + yOffSet, r1,
                             rmax-xOffSet)
                f2 = partial(circle, False,
                             1 + yOffSet + r1 + thickness - r2,
                             r2, rmax-xOffSet)
            ...
            elif(r1 > 0 and not r2):
                # plano-convex case 1 fixed
                if(not thickness):
                    thickness = abs(r1-(r1**2-diameter**2/4)**(1/2))
                rmax = r1
                f1 = partial(circle, True, 1 + yOffSet + rmax, r1,
                             rmax-xOffSet)
                f2 = partial(line, 1+yOffSet + thickness)
                d = rmax - thickness
                b = -2*(rmax+xOffSet)
                ...
                rmax = r2
                f1 = partial(line, 1 + yOffSet - thickness)
                f2 = partial(circle, True, 1 + rmax + yOffSet, r2,
                             rmax-xOffSet)
                return cls(surfaceList=abrvFunc(r1, r2, f1, f2, diameter,
                ...
                r2 = -r2
                rmax = r2
                f1 = partial(circle, False, 1 + yOffSet, r2, rmax-xOffSet)
                f2 = partial(line, 1 + yOffSet + r2 - thickness)
                d = rmax - thickness
                b = -2*(rmax+xOffSet)
                c = d**2 + (rmax+xOffSet)**2 - rmax**2
                x0 = (-b-(b**2 - 4*c)**(1/2))/2
                ...
                r1 = -r1
                rmax = r1
                f1 = partial(circle, False, 1 + yOffSet, r1,
                             rmax-xOffSet)
                f2 = partial(line, 1 + r1 + thickness + yOffSet)
                return cls(surfaceList=abrvFunc(r1, r2, f1, f2, diameter,
                                                xOffSet=xOffSet), n=nFunction)
                ...
                if(diameter == 0 or thickness == 0):
                    raise(Exception("Insufficient information provided for \
                                    ...
                                    thickness must be positive"))
                rmax = r1
                f1 = partial(circle, True, rmax + 1 + yOffSet, r1,
                             rmax-xOffSet)
                f2 = partial(circle, True,
                             rmax + 1 - thickness + r2 - r1 + yOffSet, r2,
                             rmax-xOffSet)
                if(diameter < 2*r2):
                    # reasonable diameter
                    return cls(surfaceList=abrvFunc(r1, r2, f1, f2, diameter,
                ...
                f1 = partial(circle, False, 1 + yOffSet, r1, rmax-xOffSet)
                f2 = partial(circle, False,
                             1 - thickness - r2 + r1 + yOffSet, r2,
                             rmax - xOffSet)
                if(diameter < 2*r1):
                    # reasonable diameter
                    return cls(surfaceList=abrvFunc(r1, r2, f1, f2, diameter,
                    ...
                    # unreasonable diameter
                    raise(Exception("This is an unreasonable diameter for a \
                                    meniscus concave lens. It should be less \
                                    than %d" % 2*r1))
        else:
            # probably should print inputs
    ...
    # we could take a dictionary of inputs which might be more readable but
    # that might also make the whole process more complicated
    # insofar as translating that dictionary set up into our list structure
    @classmethod
    def custom(cls, surfaceList, nFunction=lambda x: 1.5):
        return cls(surfaceList=surfaceList, n=nFunction)
    # there should probably some rhobust error checking for dealing with the
    # defined surfaces
